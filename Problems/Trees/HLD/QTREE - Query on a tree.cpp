...
struct tree {
	...
	vector<int> chainHead, chainIdx, chainSize, posInBase;
	...
	tree(int _n, int _r = 0) : n(_n), root(_r) {
		...
		chainNum = 0; ptr = 0;
		chainHead.resize(n, -1); chainIdx.resize(n); posInBase.resize(n);
	}
	...
	int query_up(int u, int v, SegTree& st) {
		...
		while (true) {
			...
			if (uchain == vchain) {
				if (u == v) { break; }		
				int qq = st.query(posInBase[v] + 1, posInBase[u]).v;	//+1 to avoid the edge above the lca
				ans = max(ans, qq);
				break;
			}

			int headU = chainHead[uchain];
			int qq = st.query(posInBase[headU], posInBase[u]);
			ans = max(ans, qq);
	...
	void query(int u, int v, SegTree& st) {
		int lca = LCA(u, v);
		int ans = query_up(u, lca, st); 
		int temp = query_up(v, lca, st);
		ans = max(ans, temp);
		cout << ans << '\n';
	}
};
	...
	while (t--) {
		...
		while (s[0] != 'D') {
			if (s[0] == 'C') {
				...
				int u1 = t.edges[eN * 2].u, v1 = t.edges[eN * 2].v;
				int lower = (t.depth[u1] > t.depth[v1] ? u1 : v1);
				segT.update(t.posInBase[lower], newVal);
			}
			else {
				int u, v; cin >> u >> v; u--; v--;
				t.query(u, v, segT);
