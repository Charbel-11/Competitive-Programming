...
struct SegTree {
	...
	int BigEqX(const int x, const int l) { return BigEqX(x, l, 1, 0, n - 1); }
	int BigEqX(const int x, const int l, const int si, const int lo, const int hi) {
		if (st[si].v < x || l > hi) { return -1; }
		if (lo == hi) { return lo; }

		const int mid = (lo + hi) >> 1;
		int c1 = BigEqX(x, l, si << 1, lo, mid); 
		if (c1 != -1) { return c1; }
		return BigEqX(x, l, si << 1 | 1, mid + 1, hi);
	}
};
