...
struct StUpdate {
	ll first = 0, ratio = 0; StUpdate() { }
	StUpdate(const ll f, const ll r) : first(f), ratio(r) {}
	StUpdate(const StUpdate &u1, const StUpdate &u2, int offset) {
		first = u1.first + u2.first;
		if (u2.first) { first += offset * u2.ratio; }
		ratio = u1.ratio + u2.ratio;
	}	
	void apply(StVal &sv, const int lo, const int hi) {	
		ll n = hi - lo + 1;
		ll add = first * n + ratio * ((n*(n - 1)) / 2);
		sv.v += add;
	}
};
...
int main() {
	...
	while (q--) {
		...
		if (op == 1) { st.update(a, b, StUpdate(1, 1)); }
