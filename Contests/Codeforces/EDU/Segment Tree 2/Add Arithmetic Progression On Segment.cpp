...
struct StUpdate {
	ll a = 0, r = 0; StUpdate() {}
	StUpdate(const ll _a, const ll _r) : a(_a), r(_r) {}
	StUpdate(const StUpdate &u1, const StUpdate &u2, const ll o) { 
		a = u1.a + u2.a + o * u2.r;
		r = u1.r + u2.r;
	}
	void apply(StVal &sv, const int lo, const int hi) {
		ll diff = hi - lo;
		sv.v += a * (diff + 1) + r * (diff*(diff + 1)) / 2;
	}
};
...
struct SegTree {
	...
	void updateLazy(const int si, const int lo, const int hi) {
		...
		if (lo != hi) {
			...
			lazy[si << 1 | 1] = StUpdate(lazy[si << 1 | 1], lazy[si], 1 + (lo + hi) / 2 - lo);
		}
		lazy[si] = StUpdate();
	}
...
int main() {
	...
	while (m--) {
		...
		if (type == 1) {
			...
			st.update(l - 1, r - 1, StUpdate(a, d));
		}
		else {
			int i; cin >> i; i--;
			cout << st.query(i, i) << '\n';
