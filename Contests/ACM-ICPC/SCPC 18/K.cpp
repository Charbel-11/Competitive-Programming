...
void findEdges(graph& g, vector<int>& h, bool rev) {
	stack<pair<int, int>> S; S.push({ h[0], (rev ? g.n - 1 : 0) });
	for (int i = 0; i < g.n; i++) {
		int idx = i; if (rev) { idx = g.n - i - 1; }
		while (!S.empty() && S.top().first < h[i]) { S.pop(); }
		if (!S.empty()) { g.add_edge(idx, S.top().second); }
		S.push({ h[i], idx });
	}
}
