...
struct graph {
	...
	int findDist(int u, int v) {
		int lca = LCA(u, v);
		return dist[u] + dist[v] - 2 * dist[lca] + nodes[lca].artPt;
	}
};
...
int main() {
	...
	vector<graph> blockTrees = g.buildBlockTrees(treeIdx, comp);
	for (auto& blockTree : blockTrees) { blockTree.preprocess(); }
	...
	while (q--) {
		...
		int tId = treeIdx[u]; 
		u = comp[u]; v = comp[v];
		int ans = blockTrees[tId].findDist(u, v);
		if (!blockTrees[tId].nodes[u].artPt) { ans++; }
		if (!blockTrees[tId].nodes[v].artPt) { ans++; }
		cout << ans << '\n';
	}
}