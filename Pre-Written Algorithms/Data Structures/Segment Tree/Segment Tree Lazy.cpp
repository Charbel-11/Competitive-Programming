...
struct StUpdate {
	...
	StUpdate(const StUpdate &u1, const StUpdate &u2) { v = u1 + u2; }	//Combine two lazy values, u2 is always the parent (which can have priority)
	operator ll() const { return v; }
...
struct SegTree {
	...
	void update(const int i, int c) {			//Non-lazy segment tree only
		int si = leaves[i];
		st[si] = StVal(c);
		for (si >>= 1; si; si >>= 1)
			st[si] = StVal(st[si << 1], st[si << 1 | 1]);
