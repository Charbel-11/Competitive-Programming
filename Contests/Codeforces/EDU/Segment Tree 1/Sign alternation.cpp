...
int main() {
	...
	for (int i = 0; i < n; i++) {
		if (i % 2 == 0) { even.update(i, nums[i]); }
		else { odd.update(i, nums[i]); }
	}
	...
	for (int i = 0; i < m; i++) {
		...
		if (op == 0) {
			int i, j; cin >> i >> j; i--;
			if (i % 2 == 0) { even.update(i, j); }
			else { odd.update(i, j); }
		}
		else {
			int l, r; cin >> l >> r;
			l--; r--;
			if (l % 2 == 0) { cout << even.query(l, r) - odd.query(l, r) << '\n'; }
			else { cout << odd.query(l, r) - even.query(l, r) << '\n'; }
		}
	}
}