...
struct tree {
	...
	StVal query_up(int u, int v, SegTree& st) {
		...
		while (true) {
			uchain = chainIdx[u];
			if (uchain == vchain) {
				//In this particular problem, I was working on edges, hence the break here and the +1 below
				if (u == v) { break; }
				StVal qq = st.query(posInBase[v] + 1, posInBase[u]).v;
				ans = StVal(ans, qq);
				break;
			}
	...
	void update_up(int u, int v, SegTree &st) {
		...
		while (true) {
			...
			if (uchain == vchain) {
				if (u == v) { break; }
				st.update(posInBase[v] + 1, posInBase[u], StUpdate(1)); //+1 to avoid the edge above the lca
				break;
			}
			int headU = chainHead[uchain];
