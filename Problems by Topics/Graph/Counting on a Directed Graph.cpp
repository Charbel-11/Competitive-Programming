...
struct tree {
	...
	tree(int _n, int _r = 0) : n(_n), root(_r) { nodes.resize(n); subtreeS.resize(n, 0); }
...
struct graph {
	...
	graph(int _n) : n(_n) {
		nodes[0].resize(n); nodes[1].resize(n);
	...
	tree getDominatorTree(int S) {
		int t = 0; dfs0(S, t);
		int initialN = n; n = t;		//all nodes unreachable by S are skipped
		buildDom();
...
int main() {
	...
	tree t = g.getDominatorTree(0);
	t.dfs(0, -1);

	ll ans = t.subtreeS[0] * (t.subtreeS[0] - 1) / 2;
	for (auto& e : t.nodes[0].edges) {
		ans -= t.subtreeS[e.v] * (t.subtreeS[e.v] - 1) / 2;
	}
