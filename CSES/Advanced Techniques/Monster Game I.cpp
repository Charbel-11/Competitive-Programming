...
int main() {
    ...
    int n; ll x; cin >> n >> x;
    vector<ll> s(n); for(auto &ss : s){ cin >> ss; }
    vector<ll> f(n); for(auto &ss : f){ cin >> ss; }

    ConvexHullTrick CHT; CHT.addPoint(x, 0);
    for(int i = 0; i < n - 1; i++){
        ll curBest = CHT.getMinDotProduct(s[i], 1);
        CHT.addPoint(f[i], curBest);
    }

    cout << CHT.getMinDotProduct(s[n-1], 1) << '\n';
}