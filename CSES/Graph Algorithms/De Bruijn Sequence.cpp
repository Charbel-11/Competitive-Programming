...
struct graph {
	...
	vector<int> euler_directed() {
		...
		vector<int> res, which(n + 1, 0);		//which[v] = index of the edge with endpoint v to check next
		stack.push_back({ start, 0 });
		while (!stack.empty()) {
			int v = stack.back().first; bool b = stack.back().second;
			while (which[v] < (int)nodes[v].edges.size() && used[nodes[v].edges[which[v]]]) { which[v]++; }
			if (which[v] == (int)nodes[v].edges.size()) {
				stack.pop_back(); res.push_back(b);
			}
			else {
				int ed = nodes[v].edges[which[v]];
				int w = (edges[ed].u) == v ? edges[ed].v : edges[ed].u;
				used[ed] = 1; stack.push_back({ w, edges[ed].w });
			}
		}
