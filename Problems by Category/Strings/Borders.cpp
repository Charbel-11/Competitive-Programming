...
ll findNumBorders(int l, int r, int rem, SegTree& st) {
	if (l >= r) { return 0; }

	StVal mi = st.query(l + 1, r);
	int idx = mi.idx;
	ll ans = 0, range = r - l + 1;

	ans += (range * (range - 1) / 2ll) * (mi.v - rem);
	rem = mi.v;
	ans += findNumBorders(l, idx - 1, rem, st);
	ans += findNumBorders(idx, r, rem, st);
...
int main() {
	...
	ll ans = findNumBorders(1, lcp.size() - 1, 0, st);
	ans += n * (n + 1) / 2ll;
	cout << ans << '\n';
}