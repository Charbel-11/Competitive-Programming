...
struct query {
	int L, R, idx, c, extra;
	query(int l, int r, int _c, int i) : L(l), R(r), c(_c), idx(i), extra(-1) {}
...
void add(int i) {
	...
	freqNode[curNode]++;
	if (freqNode[curNode] == 2) { freqVal[w]--; }
	else if (freqNode[curNode] == 1) { freqVal[w]++; }
}

void remove(int i) {
	int curNode = orderM[i]; int w = t.nodes[curNode].w;
	freqNode[curNode]--;
	if (freqNode[curNode] == 1) { freqVal[w]++; }
	else if (freqNode[curNode] == 0) { freqVal[w]--; }
}
...
int main() {
	...
	while (scanf("%d%d", &N, &M) != EOF) {
		...
		for (int i = 0; i < M; i++) {
			int u, v, c; cin >> u >> v >> c; u--; v--; comp.push_back(c);
			if (startT[u] > startT[v]) { swap(u, v); }
			int lca = t.LCA(u, v);
			if (lca == u) { queries.push_back(query(startT[u], startT[v], c, i)); }
			else {
				queries.push_back(query(endT[u], startT[v], c, i));
		...
		for (auto &q : queries) {
			...
			ans[q.idx] = (freqVal[q.c] > 0) || (t.nodes[orderM[q.extra]].w == q.c);
		}
