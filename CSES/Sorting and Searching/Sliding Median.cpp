...
int main() {
	...
	int n, k; cin >> n >> k;
	vector<int> nums(n); node *treap = NULL;
	for (auto &x : nums) { cin >> x; }

	int mid = (k % 2 ? k / 2 : k / 2 - 1) + 1;
	for (int i = 0; i < k; i++) { insert(treap, new node(nums[i])); }
	cout << findKth(treap, mid) << ' ';
	
	for (int i = k; i < n; i++) {
		insert(treap, new node(nums[i]));
		erase(treap, nums[i - k]);
		cout << findKth(treap, mid) << ' ';
	}
	cout << '\n';
}