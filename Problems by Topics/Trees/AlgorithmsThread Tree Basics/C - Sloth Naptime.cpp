...
int main() {
	...
	while (q--) {
		...
		if (t.dist(a, b) <= c) { cout << b + 1 << '\n'; }
		else {
			int lca = t.LCA(a, b);
			int aToLca = t.depth[a] - t.depth[lca];
			if (aToLca >= c) { cout << t.findKthParent(a, c) + 1 << '\n'; }
			else { 
				c -= aToLca; 
				int bToLca = t.depth[b] - t.depth[lca];
				cout << t.findKthParent(b, bToLca - c) + 1 << '\n';
			}
		}
	}
}