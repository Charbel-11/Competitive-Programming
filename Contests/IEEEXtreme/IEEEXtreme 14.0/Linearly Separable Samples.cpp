...
int main() {
	int t; cin >> t; while (t--) {
		...
		for (int i = 1; i < n && colinear; i++) if (v[i].second != v[i - 1].second)
			if ((v[i].first - v[i - 1].first) < EPS) { idx = i; colinear = true; }
