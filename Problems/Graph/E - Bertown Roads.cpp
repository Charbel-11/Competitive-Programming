...
struct graph {
	...
	vector<edge> edges;
	vector<int> bridges, spanEdges, backEdges;
	...
	void bridgeDFS(int cur, int p, vector<bool> &vis, vector<int> &low, vector<int> &tin, int &timer) {
		...
		for (auto &x : nodes[cur].edges) {
			...
			if (vis[e.v]) {
				if (tin[e.v] < tin[cur]) { backEdges.push_back(x); }
				low[cur] = min(low[cur], tin[e.v]); 
				continue; 
			}
			spanEdges.push_back(x);

			bridgeDFS(e.v, cur, vis, low, tin, timer);
			low[cur] = min(low[cur], low[e.v]);
			if (low[e.v] > tin[cur]) { bridges.push_back(x); }
		}
	}
	...
	void printEdgeOrientation() {
		for (auto &x : spanEdges) {
			auto &e = edges[x];
			cout << e.u + 1 << ' ' << e.v + 1 << '\n';
		}
		for (auto &x : backEdges) {
			auto &e = edges[x];
			cout << e.u + 1 << ' ' << e.v + 1 << '\n';
		}
	}
...
int main() {
	...
	g.findBridges();
	if (g.bridges.size() > 0) { cout << 0 << '\n'; return 0; }
	g.printEdgeOrientation();
}