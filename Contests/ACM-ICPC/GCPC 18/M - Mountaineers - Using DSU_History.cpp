...
vector<vector<pair<pair<int, int>, int>>> history;
...
int main() {
	...
	for (int k = 0; k < n*m; k++) {
		...
		if (i != 0 && on[i - 1][j]) { dsu.merge(idx(i, j), idx(i - 1, j), k, g[i][j]); }
		if (i != m - 1 && on[i + 1][j]) { dsu.merge(idx(i, j), idx(i + 1, j), k, g[i][j]); }
		if (j != 0 && on[i][j - 1]) { dsu.merge(idx(i, j), idx(i, j - 1), k, g[i][j]); }
		if (j != n - 1 && on[i][j + 1]) { dsu.merge(idx(i, j), idx(i, j + 1), k, g[i][j]); }
	}
