...
struct graph {
	...
	//Can cause some problems with multiedges (if we want to distinguish between them) same with directed edges which are added twice (in each direction). 
	//Each edge will be added twice itself so we might encounter backward edges with any of the 2 ids
	//To solve this, we can have just one edge between each u,v with aggregate capacity but keep track for each u,v of all directed edges with
	//their specific capacities; i.e., for each u,v keep a vector<edge>. In the decomposition, we can use the last edge in this vector and then pop back (if saturated)
