...
const int MIN = 0, MAX = 3 * 1000 * 100 + 2;
...
struct WaveletTree {
	...
	//Query #3: product of numbers <= k in [l, r]
	ll ProdLessThanK(int l, int r, int k) {
		if (l > r || k < lVal) { return 1; }
		if (lVal == rVal) { return power(lVal, r - l + 1) % mod; }
		if (rVal <= k) { return prod.empty() ? 1 : (prod[r] * (l ? modInv(prod[l - 1]) % mod : 1)) % mod; }

		int befL = (l ? left[l - 1] : 0);
		return (lC->ProdLessThanK(befL, left[r] - 1, k) * rC->ProdLessThanK(l - befL, r - left[r], k)) % mod;
	}
};
